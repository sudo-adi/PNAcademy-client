...
const OnGoingTabContent = () => {
  ...
  const onGoingAssessmentsList = assessmentList.filter(assessment => { return (new Date(assessment.start_at) < new Date() && new Date(assessment.end_at) > new Date()) && (assessment.is_active) });
    ...
    <>
      <Card className='my-2 max-h-[calc(100vh-18rem)] w-full flex flex-col'>
        <Table>
          ...
          <TableBody>
            {onGoingAssessmentsList.map(assessment => (
              <Row
                key={assessment.id}
      ...
      <div className="flex h-[calc(4rem-6px)] items-center justify-between gap-2">
        <Label className='text-[10px]'>
          Showing {onGoingAssessmentsList.length} Assessments
        </Label>
      </div>
    </>
  )

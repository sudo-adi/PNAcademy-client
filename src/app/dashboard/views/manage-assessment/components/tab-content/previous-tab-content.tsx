...
const PreviousTabContent: React.FC<PreviousTabContentProps> = ({ assessments, loadingAssessments, errorAssessments, toggleSorting, refreshAssessments }) => {
  // all hooks here
  ...
  const endedAssessments: Assessment[] = allAssessments.filter(assessment => assessment.is_active && assessment.end_at && new Date(assessment.end_at) < new Date());
  // all functions here
    ...
    <>
      ...
      <div className="flex h-[calc(3rem-6px)] items-center justify-between gap-2">
        <Label className='text-[10px]'>
          <strong>{endedAssessments.length}</strong>{" "}Assessments
        </Label>
      </div>
    </>
  )

...
const PreviousTabContent = () => {
  ...
  const endedAssessmentsList = assessmentList.filter(assessment => { return (new Date(assessment.end_at) < new Date()) && (assessment.is_active) })
    ...
    <>
      <Card className='my-2 max-h-[calc(100vh-18rem)] w-full flex flex-col'>
        <Table>
          ...
          <TableBody>
            {endedAssessmentsList.map(assessment => (
              <Row
                key={assessment.id}
      ...
      <div className="flex h-[calc(4rem-6px)] items-center justify-between gap-2">
        <Label className='text-[10px]'>
          Showing {endedAssessmentsList.length} Assessments
        </Label>
      </div>
    </>
  )

...
import useGroupUsersSelectTableStore from "@/lib/stores/manage-groups-store/group-user-select-table-store";
...
const EditUserDialog: React.FC<EditUserDialogProps> = ({
  ...
  // all hooks here
  const { editGroup, fetchGroupUsers, removeUsersInGroup, addUsersInGroup } =
    useGroups();
  ...
  const [selectedAddTabUsers, setSelectedAddTabUsers] = useState<Set<string>>(
    new Set()
  );
  const [currentTab, setCurrentTab] = useState<number>(0);
  const [loadingUsers, setLoadingUsers] = useState<boolean>(false);
  const [loadingUnAddedUsers, setLoadingUnAddedUsers] =
  ...
  // local vars here
  const allSelected =
    editUsersList.length > 0 && selectedUsers.size === editUsersList.length;
  const allSelectedAddTabUsers =
    unAddedUsers.length > 0 && selectedAddTabUsers.size === unAddedUsers.length;
  ...
  const fetchUnaddedUsersList = async () => {
    ...
    try {
      ...
      setUnAddedUsers(
        users.filter(
          (user) => !editUsersList.some((editUser) => editUser.id === user.id)
        )
      );
    } catch (err) {
      throw err;
          ...
          <Tabs defaultValue="group" className="w-full">
            <div className="flex w-full justify-between">
              <TabsList className="grid grid-cols-2 text-sm w-[300px]">
                <TabsTrigger value="group" onClick={() => setCurrentTab(0)}>
                  Group Users
                </TabsTrigger>
                <TabsTrigger
                  value="add"
              ...
              {removingUsers ? (
                ...
                <Button
                  ...
                  variant="destructive"
                  onClick={() =>
                    handleUserRemoveFromGroup(Array.from(selectedUsers))
                  }
                >
            ...
            <TabsContent value="group" className="w-full">
              <Card className="h-[600px] w-full overflow-y-auto p-0">
                    ...
                    <TableBody>
                      {editUsersList.map((user: SingleUser) => (
                        <Row
                          ...
                          removeUser={() =>
                            handleUserRemoveFromGroup([user.id])
                          }
                        />
                      ))}
                    </TableBody>
...
const AddUsersSchema: React.FC<AddUsersSchemaProps> = ({
  toggleSorting,
  sortBy,
  order,
  allSelected,
...
const AddUsersRow: React.FC<AddUsersRowProps> = ({
  user,
  selected,
  onSelectUser,
  loading,
    ...
    <TableRow>
      ...
      <TableCell>
        {loading ? (
          ...
          <Badge
            variant="outline"
            className="hover:bg-secondary hover:cursor-default"
          >
            {user.email}
          </Badge>
        )}
...
const Row: React.FC<RowProps> = ({
  ...
  return (
    <TableRow>
      ...
      <TableCell>
        {removingUser ? (
          <Loader className="h-4 w-4 animate-spin dark:text-red-600 text-red-400" />
        ) : loading ? (
          <Skeleton className="w-32 h-4" />
        ) : (

...
const CreateGroupDialog: React.FC<CreateGroupDialogProps> = ({
  ...
  const [fetchUsersDataError, setFetchUsersDataError] = useState<
    ApiError | Error
  >();
  const [fetchUsersDataByRoleError, setFetchUersDataByRoleError] = useState<
    ApiError | Error
  >();
  const [fetchRolesDataError, setFetchRolesDataError] = useState<
    ApiError | Error
  >();
  ...
  const fetchUsersDataByRole = useCallback(
    async (roleId: string) => {
      setLoadingUsers(true);
      try {
  ...
  const handleUserSelect = (
    ...
    setRoleTabsData((prev) => {
      ...
      updatedData.allSelected =
        updatedSelectedUsers.size === updatedData.users.length;
      return { ...prev, [roleId]: updatedData };
    });
  };
    ...
    <>
      <Dialog>
        ...
        <DialogContent className="w-[1000px] max-w-none">
          <DialogHeader>
            ...
            <DialogDescription>
              Creating group will allow you to assign multiple users to a single
              group.
            </DialogDescription>
          ...
          <DialogFooter>
            <div className="flex w-full justify-between gap-2">
              ...
              <div className="flex flex-row items-center gap-2">
                <Button
                  disabled={
                    selectedUsers.size > 0 && groupName.length > 2
                      ? false
                      : true
                  }
                  variant="default"
      ...
      <Dialog open={statusDialogOpen} onOpenChange={setStatusDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {createGroupError ? "Error" : "Group Created"}
            </DialogTitle>
          </DialogHeader>
          <DialogDescription>
            {creatingGroup ? (
          ...
          <DialogFooter>
            <DialogClose asChild>
              <Button variant="default" onClick={handleCloseStatusDialog}>
                Close
              </Button>
            </DialogClose>
          </DialogFooter>
...
const SelectedUsersSchema: React.FC<SelectedUsersSchemaProps> = ({
  toggleSorting,
  sortBy,
  order,
  allSelected,
...
const SelectedUsersRow: React.FC<SelectedUsersRowProps> = ({
  user,
  selected,
  onSelectUser,
  loading,
  ...
  return (
    <TableRow>
      ...
      <TableCell>
        {user?.role_id && Array.isArray(roles)
          ? roles.find((role) => role.id === user.role_id)?.name || "No role"
          : "No role"}
      </TableCell>
      <TableCell>
        {loading ? (
...
const UsersSchema: React.FC<UsersSchemaProps> = ({
  toggleSorting,
  sortBy,
  order,
  allSelected,
